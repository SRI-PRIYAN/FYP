...
def run_episode(env, agent, rpm):
    ...
    while True:
        ...
        next_obs, cost, done,comm,var = env.step(action)
        ...
        with open("cost1.txt", "a") as f:  
                f.write("%f,%f \n" %(comm,var))
        t_episode.append(ep)
        t_comm_list.append(comm)
        t_var_list.append(var)
    ...
    plt.plot(t_episode,t_comm_list)
    plt.title('Communication Overheard across training')
    plt.xlabel('Episode')
    plt.ylabel('Comm Overhead')
    plt.show()
    plt.plot(t_episode,t_var_list)
    plt.title('Resource variance across training')
    plt.xlabel('Episode')
...
def main():
    ...
    while episode < max_episode:  
        ...
        logging.info('episode:{} e_greed:{} Comm:{} Var:{} Cost: {} Reward:{} Action:{}'.format(
            episode, agent.e_greed, sc_comm,sc_var,np.mean(eval_totalCost),np.mean(eval_totalReward), env.action_queue))
        with open("cost2.txt", "a") as f:  
                f.write("%f,%f \n" %(comm,var))
        e_episode.append(ep)
        e_comm_list.append(comm)
        e_var_list.append(var)

    
    plt.plot(e_episode,e_comm_list)
    plt.title('Communication Overheard during testing')
    plt.xlabel('Episode')
    plt.ylabel('Comm Overhead')
    plt.show()
    plt.plot(e_episode,e_var_list)
    plt.title('Resource variance during testing')
    plt.xlabel('Episode')
    plt.ylabel('Resource Variance')
    plt.show()
